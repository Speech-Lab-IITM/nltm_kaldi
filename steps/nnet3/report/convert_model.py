...
# pickled python3 dict.  (see documentation for the function 'read_model' below
# for more details).
#
...
def read_matrix(s, pos):
   ...
   if tok != '[':
      print("{0}: at file position {1}, expected matrix but got {1}".format(
         tok).format(sys.argv[0], pos, tok), file=sys.stderr)
      return (None, pos)
   # m will be an array of arrays (python3 arrays, not numpy arrays).
   m = []
   while True:
