...
stage= 
nj= #16
data_folder= #/tts/cs20s029/kannada_asr_trial/data
exp_folder= #exp_trial
train_set= #train
test_sets= #"dev test"
gmm= #tri3_7000_140000
...
for datadir in ${train_set} ${test_sets}; do
  if [ -d $data_folder/${datadir}_sp ]; then
      rm -r $data_folder/${datadir}_sp
  fi

  if [ -d $data_folder/${datadir}_sp_hires ]; then
      rm -r $data_folder/${datadir}_sp_hires
  fi

  if [ -d $data_folder/${datadir}_hires ]; then
      rm -r $data_folder/${datadir}_hires
  fi
